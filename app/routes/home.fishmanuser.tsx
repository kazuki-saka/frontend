...
export async function loader({ request, context }: LoaderFunctionArgs) {
  console.log("======fishman_user_LOADER======");
...
export async function action({ request, context }: ActionFunctionArgs) {
  console.log("======fishman_user_ACTION======");
  // セッション取得
  const session = await getSession(request.headers.get("Cookie"));
  ...
  if (Number(formData.get("step")) === 1) {
    ...
    const _signupUserFormData = {
      ...signupUserFormData,
      username: String(formData.get("user[email]")),
    ...
    // セッションに保存
    session.set("fishman-user-form-data", JSON.stringify(_signupUserFormData));
    
    // STEP2へリダイレクト
    return redirect(`/home/fishmanuser?step=2`, {
      headers: {
        "Set-Cookie": await commitSession(session),
      },
    });
  ...
  if (Number(formData.get("step")) === 2) {
    ...
    console.log("_signupUserFormData=", _signupUserFormData);
    console.log("_signupUserFormData.personal=", _signupUserFormData.personal);
    
    // セッションに保存
    session.set("fishman-user-form-data", JSON.stringify(_signupUserFormData));
    
    // STEP3へリダイレクト
    return redirect(`/home/fishmanuser?step=3`, {
      headers: {
        "Set-Cookie": await commitSession(session),
      },
    });
  ...
  if (Number(formData.get("step")) === 3) {
    // セッションからフォームデータ取得
    const signupUserFormData = await JSON.parse(await session.get("fishman-user-form-data")) as SignupUserFormData;
